...
bind_hidden() {
	...
	"
    cd $HOME
    for i in $(echo "$BIND_HIDDEN_FILES" | xargs)
	do
	    if [ ! -L ".$i" ] ; then
	    	if [ -f ".$i" ] ; then
                sudo mv .$i .$i.bak-$(date +%Y%m%d)
	...
	"
    cd $HOME
    for i in $(echo "$BIND_HIDDEN_DIRS" | xargs)
	do
	    if [ ! -h ".$i" ] ; then
	    	if [ -d ".$i" ] ; then
                sudo mv .$i .$i.bak-$(date +%Y%m%d)
...
if [ ! -d "$PRIMARY_SYNC_FOLDER" ] ; then
    ...
    echo
    echo "Please provide a valid primary directory for your dotfiles"
    exit 1
fi
if [ ! -d "$SECONDARY_SYNC_FOLDER" ] ; then
    echo "$SECONDARY_SYNC_FOLDER is not a valid directory"
    echo
    echo "Please provide a valid secondary directory for your dotfiles"
    exit 2
fi
